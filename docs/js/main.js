...
Template Name: Jeet - Business Consulting HTML5 Template
Author:  ThemeBihar
Author URI: https://www.templatemonster.com/authors/themebihar/
Version: 1.0.0
Description: Jeet - Business Consulting HTML5 Template
...
(function ($) {
	...
	// ----------------------- header menu active js code
	function onepageHeaderLink() {
		let navigationBars = document.querySelectorAll(" .navigation");

		navigationBars.forEach((navigationBar) => {
			let onepageHeaderLinks = navigationBar.querySelectorAll("li a");

			function setActive(link) {
				onepageHeaderLinks.forEach((el) => el.classList.remove("active"));
				link.classList.add("active");
			}

			window.addEventListener("scroll", () => {
				onepageHeaderLinks.forEach((a) => {
					let targetElement = document.querySelector(a.getAttribute("href"));
					if (targetElement) {
	...
	function mobileMenu() {
		if ($(".mobile-menu").length) {
			var mobileMenuContent = $(".header-nav .navigation").html();

			$(".mobile-menu .navigation").append(mobileMenuContent);
			$('.sticky-header .navigation').append(mobileMenuContent);
			$.fn.mobilemenu = function (options) {
				var opt = $.extend({
					menuToggleBtn: ".menu-toggle, .mobile-menu ul li a:not(.submenu-item-has-children > a)",
	...
	function ajaxForm() {
		...
		const forms = document.querySelectorAll(".contact_form, .comment_form, .newsletter-form");
	...
	let typed_list = document.querySelector('#typed_list');

	if (typed_list) {
		let aboutsocial = new Typed('#typed', {
	...
	// Pricing Card current
	if ($(".single-pricing-cart").length) {
		$('.single-pricing-cart').on('mouseenter', function () {
			$(this).addClass('current');
			$('.single-pricing-cart').not(this).removeClass('current');
		});
	}
